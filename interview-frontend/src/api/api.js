...
export const addUserToProcedure = async (planId, procedureId, userIds) => {
    const url = `${api_url}/PlanProcedureUser/AddUserToProcedure`;
    var command = { planId, procedureId, userIds };
    const response = await fetch(url, {
        method: "POST",
        headers: {
            Accept: "application/json",
    ...
    if (!response.ok) throw new Error("Failed to add user to procedure");

    return true;
};

export const getProcedureUsers = async (planId) => {
